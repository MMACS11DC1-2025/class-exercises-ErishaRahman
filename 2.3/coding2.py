"""
Write an Olympic Judging program that outputs the average scores from 5 different judges. 

Each score is out of 10 points maximum. Half points are allowed (e.g. 7.5)

The program should take 5 inputs and output the final average score.

Example:

Judge 1: 5.5
Judge 2: 10
Judge 3: 7
Judge 4: 8.5
Judge 5: 9
Your Olympic score is 8.0
"""





score1 = float(input("judge one?"))
score2 = float(input("judge number"))
score3 = float(input("judge number:?"))
score4 = float(input("judge number"))
score5 = float(input("judge number:?"))

score = (score1 + score2 + score3 + score4 + score5)/5
print("your score is " + str(score))